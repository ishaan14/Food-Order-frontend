...
const MyCart = () => {
    ...
    if (cart.length === 1) {
        totalCartPrice = parseInt(cart[0].price) * parseInt(cart[0].qty);
    }
    else if (cart.length>1) {
        totalCartPrice = cart.reduce((prev, curr) => parseInt(prev.price)* parseInt(prev.qty) + parseInt(curr.price)* parseInt(curr.qty) );
    }
